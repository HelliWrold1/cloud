	...
	testFns := []gotest.RouterInfo{
		{
			...
			Path:        "/user",
			HandlerFunc: h.IHandler.(UserHandler).Create,
		},
		{
			FuncName:    "DeleteByID",
			Method:      http.MethodDelete,
			Path:        "/user/:id",
			HandlerFunc: h.IHandler.(UserHandler).DeleteByID,
		},
		{
			FuncName:    "DeleteByIDs",
			Method:      http.MethodPost,
			Path:        "/users/delete/ids",
			HandlerFunc: h.IHandler.(UserHandler).DeleteByIDs,
		},
		{
			FuncName:    "UpdateByID",
			Method:      http.MethodPut,
			Path:        "/user/:id",
			HandlerFunc: h.IHandler.(UserHandler).UpdateByID,
		},
		{
			FuncName:    "GetByID",
			Method:      http.MethodGet,
			Path:        "/user/:id",
			HandlerFunc: h.IHandler.(UserHandler).GetByID,
		},
		{
			FuncName:    "ListByIDs",
			Method:      http.MethodPost,
			Path:        "/users/ids",
			HandlerFunc: h.IHandler.(UserHandler).ListByIDs,
		},
		{
			FuncName:    "List",
			Method:      http.MethodPost,
			Path:        "/users",
			HandlerFunc: h.IHandler.(UserHandler).List,
		},
	}
...
func Test_userHandler_ListByIDs(t *testing.T) {
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("ListByIDs"), &types.GetUsersByIDsRequest{IDs: []uint64{testData.ID}})
	if err != nil {
		t.Fatal(err)
	}
	if result.Code != 0 {
	...
	// get error test
	err = gohttp.Post(result, h.GetRequestURL("ListByIDs"), &types.GetUsersByIDsRequest{IDs: []uint64{111}})
	assert.Error(t, err)
}
...
func Test_userHandler_List(t *testing.T) {
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("List"), &types.GetUsersRequest{query.Params{
		Page: 0,
		Size: 10,
